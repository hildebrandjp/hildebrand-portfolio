...
import {
    ...
    RAIN_COLOR_BRIGHT, RAIN_TRAIL_BRIGHT_MIN, RAIN_TRAIL_BRIGHT_RANGE,
...
export class MatrixRain implements IDisposable, IAnimatable {
    ...
    // drops[i] = X column-index of the leading (right-most) character for row i
    // starts beyond right edge, decreases each tick (streams left)
    ...
    constructor() {
        ...
        for (let i = 0; i < this.numRows; i++) {
            // Stagger start positions so streams flow in gradually from the right
            this.drops[i] = maxCol + Math.random() * maxCol;
        }
    ...
    private _tick(): void {
        ...
        const maxCol = canvas.width / RAIN_FONT_SIZE;

        // Fade previous frame — higher alpha = shorter tail, less accumulated brightness
        ctx.fillStyle = `rgba(7, 5, 18, ${RAIN_FADE_ALPHA})`;
        ...
        for (let i = 0; i < numRows; i++) {
            ...
            // Skip until the stream enters the canvas from the right
            if (x > canvas.width) {
            ...
            // Reset when stream exits left edge
            if (x < 0 && Math.random() > RAIN_RESET_THRESHOLD) {
                drops[i] = maxCol + Math.random() * RAIN_RESET_RANGE;
            }
        }
